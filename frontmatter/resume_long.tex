...
\section*{Expérimentation}
    ...
    \subsection*{Expériences Vanille de base}
        ...
        D'après les résultats expérimentaux, au niveau \textbf{eFin} = 3, nous avons a appris que:
        \begin{itemize}
            \item La RMSE ne permet pas de prévoir complètement les erreurs définies dans notre taxonomie;
            \item Les \texttt{Erreurs de Bâtiment} sont mieux détectées sur \textbf{Elancourt} que sur les autres scènes;
            \item \texttt{FOS} est très bien détectés avec plus de \SI{95}{\percent} de F-score, sur tous les zones d'intérêt;
            \item \`A l'exception de \texttt{FOS}, plus la zone urbaine est dense, mieux les \texttt{Erreurs de Facettes} sont détectées;
            \item Les erreurs rares telles que \texttt{BIT} et \texttt{FIT} sont mal détectées sur toutes les zones urbaines;
            \item Pour la détection des erreurs, les attributs géométriques seules sont, avec une faible marge, aussi bonnes que les autres attributs extrinsèques;
            \item Le type de bâtiment est donc un bon indicateur pour la détection d'erreurs dans une même zone;
            \item \texttt{BUS} est la seule erreur pour laquelle les autres modalités sont meilleures avec un écart important;
            \item Bien que, dans la plupart des cas, elles n'améliorent pas la prédiction des erreurs, les modalités basées sur des données externes sont aussi importantes que les caractéristiques géométriques.
        \end{itemize}

        Aux niveaux 2 et 1 de l'\textbf{eFin}, nous avons aussi prouvé expérimentalement que:
        \begin{itemize}
            \item Les erreurs de facettes sont détectées avec un F-score d'environ \SI{90}{\percent} non affecté par la scènes.
            \item Comme nous l'avons vu précédemment au niveau 3 de l'\textbf{eFin}, les \texttt{Erreurs de Bâtiment} sont difficiles à entraîner sur \textbf{Nantes} et \textbf{Paris-13}, avec un F-score d'environ \SI{75}{\percent}.
            \item Nous le confirmons une fois de plus que:
                \begin{itemize}
                    \item L'apprentissage des \texttt{Erreurs de Bâtiment} est plus difficile à transférer que les erreurs à facettes;
                    \item \textbf{Elancourt} est le meilleur choix pour l'apprentissage des \texttt{Erreurs de Bâtiment};
                    \item \textbf{Nantes} et \textbf{Paris-13} ont une composition similaire.
                \end{itemize}
            \item \SI{20}{\percent} est suffisant pour l'apprentissage sur l'ensemble de données mixtes.
            \item Comme prévu, la distinction entre \texttt{Erroné} et \texttt{Valide} s'est avérée difficile à prédire.
        \end{itemize}
