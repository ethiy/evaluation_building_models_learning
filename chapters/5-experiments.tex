...
\section{Datasets}
    ...
    \subsection{Urban scenes and modeling techniques}
        ...
        Figure~\ref{fig::elancourt_ortho} depicts the small city of \textbf{Elancourt} which contains diverse of building types: residential areas with gable and hip roof buildings and large industrial flat roof building based districts (\textit{cf.} Figure~\ref{subfig::elancourt_samples}).
        \textbf{Nantes}, as shown in Figure~\ref{fig::nantes_ortho}, represents a denser urban setting but with a lower building diversity (\textit{cf.} Figure~\ref{subfig::nantes_samples}).
        In Figure~\ref{fig::paris-13_ortho}, one can see how \textbf{Paris-13} consists of mostly flat roof high towers which coexist with Haussmann style buildings that typically exhibit highly fragmented roofs (\textit{cf.} Figure~\ref{subfig::paris-13_samples}).
...
\section{The experimental set-up}
    ...
    Next, in Section~\ref{subsec::experiments::setup::classification}, we discuss in great detail how the classification process was conducted.
    ...
    \subsection{Feature configurations}
        ...
        \subsubsection{Baseline features}
            ...
            Since we compute differences between observed and model height at terrain level also (\textit{cf.} Section~\ref{subsec::learned_evaluation::baseline::height}), and because these are virtually unbounded and can differ from one scene to another, the maximum possible discrepancy is limited to \SI{\pm 50}{\m} for all scenes.
    ...
    \subsection{Classification settings}
        \label{subsec::experiments::setup::classification}
            ...
            Herein, are given extensive details how the considered classifiers (\textit{cf.} Section~\ref{subsec::learned_evaluation::classification::classifiers}) are applied.
        ...
        \subsubsection{Classifiers}
            ...
            \paragraph{\acrshort*{acr::rf}}
                ...
                Regarding \gls{acr::scatnet} based features, in Section~\ref{subsec::more_experiments::richer_features::scatnet_baseline} were reduced using \gls{acr::pca} and Kernel\footnote{The used kernel is that same the one used with the \gls{acr::svm} classifier.} \gls{acr::pca} in order to not overwhelm the geometric features.
...
%     In Section~\ref{sec::experiments::setup} provides all details about our experimental approach.
%     Next in Section~\ref{sec::experiments::baseline_feature_analysis}, we report some first results using the baseline features we developped.
...
\section{Baseline feature analysis}
    ...
    \subsection{Feature ablation study}
        ...
        \begin{table}[htpb]
            ...
            \begin{center}
                \begin{tabular}{| c | x{1cm} x{1cm} x{1cm} x{1cm} |}
                    \hline
