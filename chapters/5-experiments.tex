...
\section{\textsc{Datasets}}
    ...
    \subsection{\textsc{Error statistics}}
        ...
        These statistics are first analysed depending on the family errors at the \texttt{finesse} = 2 level (\textit{cf.} Subfigure~\ref{subfig::family_errors}).
...
\section{\textsc{Pipeline evaluation}}
    ...
    \subsection{\textsc{Experimental set-up}}
        ...
        \subsubsection{\textsc{Considered labels}}
            All \texttt{finesse} levels were tested.
            The input models are generalized to \gls{acr::lod}-2.
            As a consequence, we chose \texttt{\gls{acr::elod}} $=$ \gls{acr::lod}-2.
            If the \texttt{exclusivity} is \textsc{on}, at \texttt{finesse} level 3, the second stage classification results depends on the first one.
            We are, at this stage, trying to prove the feasability, in the first place, of the proposed approach.
            As proven in the latter experiments (\textit{cf.} Section~\ref{}), the first stage results are not good enough to test this configuration.
            That is why we limited the experiments to the case where the \texttt{exclusivity} is set to be \textsc{off}.
    ...
    \subsection{\textsc{Baseline feature analysis}}
        ...
        \subsubsection{\textsc{\acrshort*{acr::rmse} predictive capacity}}
            ...
            Mean test results are shown in Table~\ref{tab::rmse_prediction}.\\
        ...
        \subsubsection{\textsc{Feature ablation study}}
            ...
            \begin{figure}[htpb]
                ...
                \ffigbox[\FBwidth]{
                    \begin{subfloatrow}[2]
                        \ffigbox[\FBwidth]{
                            \includestandalone[mode=buildnew, width=.45\textwidth]{figures/results/ablation/f-scores_f3_building}
                        }{
                            \caption{
                        ...
                        \ffigbox[\FBwidth]{
                            \includestandalone[mode=buildnew, width=.45\textwidth]{figures/results/ablation/f-scores_f3_facet}
                        }{
                            \caption{
            ...
            Figure~\ref{fig::f_score_ablation_f3} shows all \texttt{Building errors} family labels are better detected for \textbf{Elancourt}.
