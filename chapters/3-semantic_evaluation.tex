...
In section~\ref{sec::semantic_evaluation::overhead}, is detailed an implementation of the 
...
overhead
        At a first level, model qualifiability is studied.overhead
        In fact, aside from formatting issues or geometric inconsistencies~\parencite{ledoux2018val3dity}, other reasons make building models unqualifiable.overhead
        For instance, buildings can be occluded by vegetation and thus cannot be assessed with most of the remote sensing data sources.overhead
        Generally speaking, input models can be impaired by some pathological cases that are outside our evaluation framework.overhead
        In consequence, \texttt{Qualifiable} models are distinguished here from \texttt{Unqualifiable} buildings.overhead
        ...
        It is the lowest semantization level at which the evaluation of a model is expressed.overhead
        Then, a model is either \texttt{Valid} or \texttt{Erroneous}.overhead
        ...
        Due to the lack of sufficient observations, we did not make this choice in order to guaranty the generalizability of the taxonomy.overhead
...
\section{\textsc{Application to the geospatial overhead case}}
    \label{sec::semantic_evaluation::overhead}overhead
    ...
    \subsection{\textsc{Discussion}}
        ...
        \subsubsection{\textsc{Error taxonomy properties}}
            ...
            In fact, if we imagine a bi-level roof of a building being modeled, a misestimation of the ridge of the building is always a result of a misestimation of the slope of the facets corresponding to the two roof levels.
            The contrary is not necessarily true if one considers ????.

            This could work for facades also not only overhead.
...
\section{\textsc{Parametric label extraction}}
    ...
    \subsection{\textsc{Evaluation labels}}
        ...
        \begin{equation}
            ...
            \begin{aligned}
                \text{children}: V \setminus L &\rightarrow V\\
                n &\mapsto \{m \in V : (n, m) \in E \}
            \end{aligned}.
        \end{equation}
        where:
        \begin{conditions}
