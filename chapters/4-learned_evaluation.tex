...
All related issues are detailed in section~\ref{sec::learned_evaluation::classification}.
Next, in section~\ref{sec::learned_evaluation::baseline}, is presented a baseline of building \gls{acr::3d} model features.
These are used to describe models and are fed to the supervized classifier.
Third and last, richer extracted features are discussed in section~\ref{sec::learned_evaluation::richer_features}.
These are proposed to better predict errors in the model.
    ...
    \subsection{\textsc{Different classification problems}}
        ...
        \begin{table}[htbp]
            ...
            \begin{tabular}{c c c x{11cm}}
                ...
                2 & \gls{acr::lod}-2 & \textsc{on} & MultiClass(\texttt{Valid}, \texttt{Building Errors}, \texttt{Facet Errors})\\
                2 & \gls{acr::lod}-2 & \textsc{off} & MultiLabel(\texttt{Building Errors}, \texttt{Facet Errors})\\
                3 & \gls{acr::lod}-1 & \textsc{on} & MultiLabel(children(Binary(\texttt{Valid}, \texttt{Building Errors})))\\
                3 & \gls{acr::lod}-2 & \textsc{on} & MultiLabel(children(MultiClass(\texttt{Valid}, \texttt{Building Errors}, \texttt{Facet Errors})))\\
                3 & \gls{acr::lod}-1 & \textsc{off} & MultiLabel(children(\texttt{Building Errors}))\\
                3 & \gls{acr::lod}-2 & \textsc{off} & MultiLabel(children(\texttt{Building Errors})$\cup$ children(\texttt{Facet Errors}))\\
                \bottomrule
            \end{tabular}
            \caption{
        ...
        In Table~\ref{tab::classification_problems}, Binary(\texttt{Valid}, $l$) refers to the binary setting that predicts the most probable class \texttt{Valid} or $l$.
        MultiClass($l_1, l_2, \dots, l_c$) (\textit{resp}. MultiLabel($l_1, l_2, \dots, l_c$)) corresponds to the multi-class (\textit{resp}. multi-label) setting.
        We note that: MultiLabel(\texttt{Valid}, $l_1, l_2, \dots, l_c$)) = MultiLabel($l_1, l_2, \dots, l_c$).
        Two consecutive classification problems relate to a hierchical classification:
        depending on the class that is predicted in the first stage, a second classification problem chooses between corresponding labels.\\
...
\section{\textsc{Feature baseline}}
    ...
    In order to detect labels previously defined in chapter~\ref{chap::semantic_evaluation}, we formulate the problem as a supervized learning classification problem.
    Based on annotated 
     while guarantying a certain flexibility towards reference data, multiple modalities are necessary.
    The structure of the 3D model can be directly used to extract geometrical features.
    Dense depth information can be added, through for instance a \gls{acr::dsm}, so as to help detecting geometric defects that can be hardly discriminated otherwise (as in Figure~\ref{fig::fac_err}.(e)), in particular for the outer part of buildings.
    VHR optical images bring additional information (high frequencies and texture) that is particularly suited for inner defect detection.
