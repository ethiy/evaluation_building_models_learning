    ...
    \subsection{\textsc{Different classification problems}}
        ...
        \begin{table}[htbp]
            ...
            \begin{tabular}{c c c x{11cm}}
                ...
                3 & \gls{acr::lod}-1 & \textsc{off} & $multi\_label(\text{children}(\texttt{Building Errors}))$\\
                3 & \gls{acr::lod}-2 & \textsc{off} & $multi\_label(\text{children}(\texttt{Building Errors})\cup \text{children}(\texttt{Facet Errors}))$\\
                \bottomrule
            \end{tabular}
            \caption{
        ...
        This denoted by:
        \begin{equation*}
            multi\_stage((l_1, l_2, \dots, l_3)) \equiv multi\_label(\text{children}(multi\_class(l_1, l_2, \dots, l_3))).
        \end{equation*}
...
\section{\textsc{Feature baseline}}
    ...
    \subsection{\textsc{Geometric features}}
        ...
        This is countersteped by looking, for each graph edge $e=\{f, g\} \in \mathsf{E}$, for the distance between facet centroids $\Vert \mathscr{G}(f) - \mathscr{G}(g) \Vert$ and the angle formed by their normals $\arccos(\vec{n}(f) \cdot \vec{n}(g))$.
    ...
    \subsection{\textsc{Height based features}}
        ...
        It must have been produced around the same time of the \gls{acr::3d} reconstruction so as to avoid temporal discrepancies.
        ...
        It is then rasterized into an image $alt \in \mathbb{R}^{w\times h}$ using the same spatial resolution as $dsm$.
        Their difference between the two height grid provides a discrepancy map (Figure~\ref{fig::graphical_abstract}.c).
        ...
        \begin{figure}[H]
            ...
            \caption{
                ...
                (a) \glspl{acr::dsm}.
                (b) Height maps extracted from the \gls{acr::3d} model.
    ...
    \subsection{\textsc{Image based features}}
        ...
        In this paper, we only used orthorectified images., while not being a raw sensor data and riddled with artifacts, are more accessible
        ...
        The same can be done over all facets of a building $\mathsf{M}$ (Equation~\ref{eq::corr_bul}). The weights are added in order to take into account the geometry heterogeneity. The gradient to normal comparison is similar to the \gls{acr::3d} data fitting term formulated in~\parencite{li2016boxfitting}. Once again, the model structure is partially lost when simply summing histograms over all segments.
        \begin{equation}
            \label{eq::corr_bul}
