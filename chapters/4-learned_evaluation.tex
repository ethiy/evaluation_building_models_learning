        ...
        \subsubsection{Image based features}
            ...
            \begin{description}
                \item[\texttt{Deletion}:] Pixels \(g\) in the corresponding orthoimage which are part of a polygon border (i.e., \(Q_{\mathsf{M}}(g) = 1\)) are made black, as shown in Figure~\ref{fig::deletion}:
                        ...
                        where:
                        \begin{conditions}
                            J_{h_{\mathsf{M}}, w_{\mathsf{M}}} = \left(1\right)_{\substack{i \in \llbracket 1, h_\mathsf{M} \rrbracket\\j \in \llbracket 1, w_\mathsf{M} \rrbracket}} & is the matrix of ones of size \(h_{\mathsf{M}} \times w_{\mathsf{M}}\).\\
                            P^{\otimes 3} = P \otimes P \otimes P & \makecell{is the tensor obtained by stacking in depth three copies of\\the same matrix \(P\);}\\
                            A \odot B  = \left(A_{ij} \cdot B_{ij} \right)_{ij} & \makecell{denotes the Hadamard/Schur product of any two matri-\\ces \(A\) and \(B\).}
                        \end{conditions}
