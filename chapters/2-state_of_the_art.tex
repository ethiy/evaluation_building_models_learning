...
Presented herein is a tour of the different notions necessary for the understanding of the present manuscript.
...
Secondly, we present a complete state-of-the-art survey on quality evaluation for \gls{acr::3d} building models (\textit{cf.} Section~\ref{sec::state_of_the_art::quality}).
...
\section{Automatic building \texorpdfstring{\gls*{acr::3d}}{3D} modeling}
    ...
    In this work, we study quality evaluation of large-scale urban modeling.
    ...
    Automatic building \gls{acr::3d} modeling consists mainly on two steps.
    ...
    \subsection{Modeling strategies}
        ...
        \subparagraph{Model-driven approaches}
            This type of methos, also called top-down, rely on strong assumptions on the type of buildings to be modeled.
        ...
        \subparagraph{Data-driven approaches}
            They can also be called bottom-up.
        ...
        \subparagraph{Limitations of current modeling methods}
            ...
            This has not been widely studied~\parencite{lafarge2012creating,bao2013generating}, as buildings are moslty modeled one by one and and most of the time not in challenging\footnote{
                Like buildings delimited by narrow streets or ones with complex architectures.
            } areas.
            All these issues and more induce defects that should be characterized and classified for a better understanding of the field and, subsequently, objectively understand which methods are the most adapted for which areas.
...
\section{Quality evaluation of \texorpdfstring{\gls*{acr::3d}}{3D} building models}
    ...
    \subsection{Types of reference data}
        ...
        \subsubsection{High resolution ground truth}
            ...
            ~\textcite{zebedin2008fusion} uses such a method to produce their reference data from aerial images with an accuracy up to the order of \SI{\pm 0.15}{\m}.
        ...
        \subsubsection{Remote sensing data}
            ...
            \subparagraph{Aerial images}
                ...
                Facets were also evaluated, in~\parencite{boudet2006supervised}, based on correlation functions computed from multiple overlapping oblique images.
    ...
    \subsection{Novelty of the proposed method}
        ...
        Conversely, the need for reasonably available reference data, which is dictated by the \textbf{large-scale} constraint, implies the reliance on remote sensing data based methods (\textit{cf.} Section~\ref{subsec::state_of_the_art::quality::reference}).\\
        ...
        Our work relies on the same idea, but goes farther by allowing the possibility to evaluate building models without using any reference data and relying on their intrinsic attributes instead.
        ...
        In fact, we propose a a novel hierarchical and modular taxonomy that assembles all possible semantic and geometric errors that can affect the building model.
        Based on this taxonomy, depending on the end user needs, error labels are extracted for the classification.
        We also developped a baseline of features, intrinsic as well as extrinsic\footnote{If available.}, which describe the evaluated models.
        Using these features so as to train some regular off-the-shelf classifiers, we proved the feasability of our approach.
        Finally, we made use of some advanced feature extractors in the aims of imporving the prediction scores of the proposed pipeline.
    ...
    \subsection{Supervised classifiers}
        ...
        \subsubsection{\acrlong*{acr::svm}}
            ...
            \paragraph{Hard margin}
                ...
                \begin{figure}
                    \centering
                    \includestandalone[mode=buildnew, width=.7\textwidth]{figures/svm/linear_separable}
                    \caption[
            ...
            \paragraph{Soft margin}
                ...
                \begin{figure}
                    \centering
                    \includestandalone[mode=buildnew, width=.7\textwidth]{figures/svm/non_linear_separable}
                    \caption[
    ...
    \subsection{Feature extraction}
        ...
        \subsubsection{Kernels for graph classification}
            ...
            \paragraph{Random walk kernel}
                ...
                Secondly, these kernels suffer from tottering which is the fact that the random walk is mostly consistent of multiple movements between a small subset of vertices in a row.
