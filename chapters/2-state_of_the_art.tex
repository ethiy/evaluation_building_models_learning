...
\section{\textsc{Supervised learning and pattern recognition}}
    ...
    \subsection{\textsc{Feature extraction}}
        ...
        \subsubsection{\textsc{\acrlong*{acr::scatnet} for image classification}}
            ...
            \paragraph{\gls*{acr::scatnet}: a reverse engineered \gls*{acr::cnn}}
                ...
                \subparagraph{Wavelets filter bank}
                    ...
                    A finite number $L$ of orientations are possible for angle indices: $\theta \in \{\frac{l\cdot\pi}{L}: l=1,2\dots,L\}$.
                        ...
                        \ffigbox{
                            ...
                            \ffigbox[\FBwidth]{
                                \begin{tabular}{@{}l@{}}
                                    ...
                                    \includegraphics[width=\textwidth]{images/related_work/second_layer_cnn}
                                \end{tabular}
                            }{
                ...
                \subparagraph{The wavelet-modulus operator}
                    ...
                    \begin{equation}
                        ...
                        W: x \mapsto \left(x\star\psi_{i, \theta}\right)_{\substack{i=1, 2 \dots, I \\ \theta \in \{\frac{l\cdot\pi}{L}: l=1,2\dots,L\}}},
                    \end{equation}
                    if it takes two variables $(u, \omega) \in \mathbb{R}^2 \times [0, 2\cdot\pi)$ it becomes:
                    \begin{equation}
                        \label{eq::wavelet_transform_2}
                        \widetilde{W}: x \mapsto \left(x\ostar_{SE(2)}\psi_{i, \theta, \iota}\right)_{\substack{i=1, 2 \dots, I \\ \theta \in \{\frac{l\cdot\pi}{L}: l=1,2\dots,L\}\\\iota=1,2\dots,\lfloor\log_2(L)\rfloor}}.
                    \end{equation}
        ...
        \subsubsection{\textsc{Kernels for graph classification}}
            ...
            Standard machine learning practice usually assumes that the data to be classified lives in a finite dimensional space.
            This is not the case for graphs in general.
            In fact, Graphs can have varying numbers of nodes and edges.
            Moreover, graphs can be different but provide the same structural information: we say they are isomorphic.
            A valid representation for graphs should take care of these two issues: incorporate all possible graph sizes and be invariant to graph isomorphisms.
            As accustomed in statistical learning, one way to alleviate representation issues for data is to directly compare graphs using kernels as seen previously in sub-subsection~\ref{subsubsec::state_of_the_art::mlpr::classifiers::svm}.\\
            
            This sub-subsection does aim at presenting a thourough survey of graph kernels.
            The work of~\textcite{ghosh2018journey} presents a structured way to look at the different graph kernels that exist depending on the used methodology.
            A different approach is proposed in~\textcite{kriege2019survey} where they study graph kernels based based on the underlying graphs.\\

            Apart from the structure of the graph, kernels should take into account the labels or continuous attributes of the graph.
            These could be given at node level or edge level.
            We will present in hereafter the graph kernels that were used for our case.
            This concerns continuously node attributed graph kernels as well as ones that cares only for the structural properties of the graph.\\

            \paragraph{Basic kernel}
                Let $G = \left(V, E\right)$ an undirected graph with vertices $v\in V$ and edges $e \in E =\{\{u, v\}: (u, v) \in V\times V\}$.
                We can give attributes to each node $a: V \rightarrow \mathbb{R}^d$ or edges $b: E \rightarrow \mathbb{R}^{d'}$.\\
                ...
                This will be described in details in subsection~\ref{subsec::learned_evaluation::baseline::geometric}.
            
            \paragraph{Random walk kernel}
                Suppose we want to compute the similarity between two graphs $G = \left(V, E\right)$ and $G' = \left(V', E'\right)$.
                We start by defining the direct product graph $G_{\times} = \left(V_{\times}, E_{\times}\right)$ defined as:
                \begin{align}
                    \label{eq::direct_product_graph}
                    V_{\times} &\triangleq V \times V'\\
                    E_{\times} &\triangleq \{\{(u, u'), (v, v')\}: \{u,v\} \in E \wedge \{u',v'\} \in E'\}.
                \end{align}
                ...
                \end{figure}
                A graph $G$ could be represented by an adjacency matrix $A$:
                \begin{equation}
                    \label{eq::adjacency_matrix}
                    A \triangleq \left(\delta_{\{u,v\}\in E}\right)_{(u, v) \in V\times V}
                \end{equation}
                We can also define a diagonal matrix of node degrees $D\triangleq \operatorname{diag}\left(\sum_{v \in V}A_{uv}\right)_{u \in V}$.
                We can hence defined a normalized matrix:
                \begin{equation}
                    \label{eq::normalized_adjacency_matrix}
                    P \triangleq A\cdot D^{-1}
                \end{equation}
                Actually, The latter can be interpreted as a transition probability matrix of a random walk on the graph:
                ...
                $p$ and $q$ are usefull to encode prior information of the graph~\parencite{vishwanathan2010graph}.\\
                ...
                A random walk helps discover the structure of the graph.
                In consequence, a random walk on the direct product graph can be a good indicator of the similarity in structure of both graphs $G$ and $G'$.
                It is defined as:
                \begin{equation}
                    \label{eq::random_walk_graph}
                ...
                The choice of the series $\left(\lambda_k\right)_{k\in \mathbb{N}}$ is critical, as the kernel defined in Equation~\ref{eq::random_walk_graph} can diverge.
                It is taken to be non-negative in order in ensure the positive semi-definite of the kernel.\\

                Setting $p_{\times} \approx 1$ and $q_{\times} \approx 1$ and , two special cases are of interest.
                \begin{itemize}
            ...
            \paragraph{\gls*{acr::svm} $\vartheta$ kernel}
                This is also a kernel takes advantage only of the structure of graphs.
                It relies on the definition of Lovasz number of a graph $\vartheta(G)$.
                We say
                \begin{equation}
                    \label{eq::orthonormal_representation}
                    W(G) \subset \{\bm{w}_v: v \in V \wedge \bm{w}_v\in\mathbb{R}^d \wedge \Vert\bm{w}_v\Vert = 1\}
                \end{equation}
                is an orthonormal representation of a graph $G$ if and only if $\forall \{u,v\} \notin E \Rightarrow \bm{w}_u^T\cdot \bm{w}_v=0$.
                ...
                Equally we define the same number over a subset of vertices $B \in V$ as follows:
                \begin{equation}
                ...
                \begin{equation}
                    ...
                    k_{Lo}(G, G') \triangleq \sum_{\substack{B\subseteq V\\B'\subseteq V'\\\vert B \vert = \vert B' \vert}} \frac{1}{
                        \begin{pmatrix}
                            \vert V \vert\\
                            \vert B \vert                            
                ...
                We define $L \triangleq \{K \in S_{\vert V \vert}^+: \forall v \in V, K_{vv} = 1 \wedge \forall \{u, v\}  \notin E, K_{uv} = 0\}$ where $S_{\vert V \vert}^+$ is the set of $\vert V \vert \times \vert V \vert$ positive semi-definite matrices.
                We can hence write:
                \begin{equation}
                ...
                It is even more interesting as for Erdos–R\'enyi random graphs $\omega(K_{LS})$ is a constant factor approximation to the Lovasz number~\parencite{jethava2013lovasz} with high probability.
