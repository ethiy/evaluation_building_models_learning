    ...
    \subsection{\textsc{Feature extraction}}
        ...
        \subsubsection{\textsc{Kernels for graph classification}}
            ...
            \paragraph{Graph hopper kernel}
                ...
                A path \(\pi\) between two vertices \((v_s,v_e) \in V\times V\) is a sequence of nodes \(\left(v_i\right)_{i=1,2\dots,\vert \pi \vert}\) such that:
                \begin{equation*}
                    \begin{cases}
                        \forall i=1,2\dots,\vert \pi \vert-1,\; \{v_i, v_{i+1}\} \in E\\
                        \forall i=1,2\dots,\vert \pi \vert,\; \pi(i) \triangleq v_i\\ 
                        v_1 = v_s\\
                        v_e = v_{\vert \pi \vert}
                    \end{cases}
                \end{equation*}
                ...
                \begin{equation}
                    ...
                    k_p(\pi, \pi') \triangleq \sum_{i=1}^{\vert \pi \vert} \kappa\left(\pi(i), \pi'(i)\right) \cdot \mathbb{1}_{\vert \pi \vert = \vert \pi' \vert}
                \end{equation}
                where:
                \begin{conditions}
                    \kappa & is a base kernel between nodes.
                \end{conditions}
                This kernel compares paths with similar length by hopping along them simultaneously.

                Based on this kernel, one can compare two graphs \(G\) and \(G'\) by comparing paths from both:
                \begin{equation}
                    \label{eq::graph_hopper_kernel}
                    k_{gh}(G, G') \triangleq \sum_{\substack{\pi \in \mathscr{P}\\\pi' \in \mathscr{P}'}} k_p(\pi, \pi').
                \end{equation}
                This kernel can be computed efficiently as it can be trandformed into:
                \begin{equation}
                ...
                where:
                \begin{conditions}
                    w(v, v') & is the number of times \(v\) and \(v'\) appear at the same coordinate \(i\) of some shortest paths \(\pi\) and \(\pi'\) with the same length \(\vert \pi \vert = \vert \pi' \vert\).
                \end{conditions}

                Let
                \begin{equation*}
                    \delta \triangleq \max_{\pi \in \mathscr{P}} \vert \pi \vert
                \end{equation*}
                the maximal length of shortest paths in \(G\).
                Define also matrix
                \begin{equation*}
                    M_G(v) \triangleq \left(\left\lvert\left\{\pi \in \mathscr{P}: \pi(i) = v \wedge \left\lvert\pi\right\rvert = j \right\}\right\rvert\right)_{\substack{i=1,2\dots,\delta\\j=1,2\dots,\delta}}
                \end{equation*}
