        ...
        \subsubsection{\textsc{\acrlong*{acr::scatnet} for image classification}}
            ...
            \paragraph{Mathematical notations}
                ...
                \subparagraph{Group convolution}
                    A convolution over a Lie group $G$ is defined as:
                    \begin{equation}
            ...
            \paragraph{\gls*{acr::scatnet}: a reverse engineered \gls*{acr::cnn}}
                ...
                \subparagraph{Wavelets filter bank}
                    ...
                    In practice, $\psi$(\textit{resp}. $\bar\psi$) are taken as two (\textit{resp}. one) dimensional Morlet wavelets, while $\phi$ is taken as a Guassian~\parencite{oyallon2015deep,sifre2013rotation,bruna2013invariant}.
                    A finite number $L$ of orientations are possible for angle indices: $\theta \in \{\frac{l\cdot\pi}{L}: l=1,2\dots,L\}$.
                    In Figure~\ref{fig::morlet_vs_learned}, we can see that the 
                ...
                \subparagraph{The wavelet-modulus operator}
                    ...
                    \begin{align}
                        ...
                        \forall (i, \theta)\{1, 2 \dots, I\} \times \{\frac{l\cdot\pi}{L}: l=1,2\dots,L\} \quad U_{i, \theta}: x \mapsto \vert x\star\psi_{i, \theta}\vert\\
                        \forall (i, \theta, \iota)\{1, 2 \dots, I\} \times \{\frac{l\cdot\pi}{L}: l=1,2\dots,L\} \times \{1,2\dots,\lfloor\log_2(L)\rfloor\} \quad \widetilde{U}_{i, \theta, \iota}: x \mapsto \vert x\ostar_{SE(2)}\psi_{i, \theta, \iota} \vert
                    \end{align}
                ...
                \subparagraph{The average pooling}
                    ...
                    These actually are invariant to translation and rotation:
                    \begin{align}
                        \label{eq::invariance_pooling}
                        \phi_I = \phi_I \circ L_{t_v}\\
                        \tilde{\phi}_I = \tilde{\phi}_I \circ L_{g_{v, \vartheta}}.
                    \end{align}
                    ...
                    \begin{equation}
                        ...
                        \tilde{x} \ostar_{SE(2)} \tilde{\phi}_I (u, \omega).
                    \end{equation}
                    ...
                    To the image, is applied the wavelet-modulus operator giving $U_{i_1, \theta_1}$.
                    The latter is pooled using the second operation~\ref{eq::pool_bi-dim-angular}.
                    This defines the second layer of scattering coefficients:
                    \begin{equation}
                        \label{eq::scatter_first}
                        S_1[x](u, i_1, \theta_1) \triangleq U_{i_1, .} \ostar_{SE(2)} \tilde{\phi}_I(u, \theta_1).
                    \end{equation}
