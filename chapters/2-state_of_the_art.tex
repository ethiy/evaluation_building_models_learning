...
\section{\textsc{Supervised learning and pattern recognition}}
    ...
    \subsection{\textsc{Supervised classifiers}}
        ...
        Classes are modeled by a finite set bijective to the set $\forall i=1,2,\dots,n \;, y_i \in \left\{1,2,\dots,C\right\}$.
        ...
        In the special case of binary classification, only two classes are possible $\forall i=1,2,\dots,n \;,\allowbreak y^i \in \left\{0, 1\right\}$.\\
        
        On another hand, the multi-label classification problem is the case where to each instance corresponds a number $L$ of binary labels: \textit{i.e.} $\forall i=1,2,\dots,n \;, y^i = (y^i_1, y^i_2, \dots, y^i_L) \in \left\{0, 1\right\}^L$.
        ...
        \subsubsection{\textsc{\acrlong*{acr::svm}}}
            ...
            \paragraph{Hard margin}
                ...
                We verify that:
                \begin{equation*}
                    \forall \lambda \in \mathbb{R}\setminus\left\{0\right\} \quad (\bm{w}, b) \in  Sol \Rightarrow (\lambda . \bm{w}, \lambda.b) \in Sol.
                \end{equation*}
    ...
    \subsection{\textsc{Feature extraction}}
        ...
        \subsubsection{\textsc{Kernels for graph classification}}
            ...
            \paragraph{Basic kernel}
                ...
                \begin{align}
                    \label{eq::feature_graph_kernels}
            ...
            \paragraph{Random walk kernel}
                In order to define this kernel, we start defining the direct product graph $G_{\times} = \left(V_{\times}, E_{\times}\right)$ where:
                \begin{align}
                ...
                \end{figure}
                We also need a definition of an adjacency matrix $A$ of a graph $G$:
                \begin{equation}
                ...
                We finally define the normalized adjacency matrix as:
                \begin{equation}
            ...
            \paragraph{\gls*{acr::svm} $\vartheta$ kernel}
                ...
                \end{equation}
                is an orthonormal representation of a graph $G$ if and only if $\forall \left\{u,v\right\} \notin E \Rightarrow \bm{w}_u^T\cdot \bm{w}_v=0$.
            ...
            \paragraph{Multiscale Laplacian kernel}
                ...
                The aplacian is not invariant to permutations as the graph.
                ...
                To that extent, based on a base kernel $\kappa$ on node attributes, first are defined the Gram matrix $K=\left(\kappa\left(u, v\right)\right)_{(u,v) \in V \cup V'}$ and $\left\{ (\lambda_1, e_1), (\lambda_2, e_2)\dots,(\lambda_p, e_p) \right\}$ all \footnote{$p \leq \vert V \vert + \vert V' \vert$} its eigenvalues and their eigenvectors such that $\forall i = 1, 2 \dots, p\; \lambda_i > 0$.
                We also need to define: $\widetilde{Q} = \begin{bmatrix}
                    \lambda_1^{\frac{1}{2}}\cdot e_1, \lambda_1^{\frac{1}{2}}\cdot e_2 \dots, \lambda_1^{\frac{1}{2}}\cdot e_p
                \end{bmatrix} \in \mathbb{R}^{p \times p}$.
                For the first graph the first $\vert V \vert$(\textit{resp.} the last $\vert V' \vert$) rows of $\widetilde{Q}$ are taken to from $Q$ (\textit{resp.} $Q'$).
                Both these matrices are needed to define $\bar{S} = Q^T\cdot L ^{-1}\cdot Q + \gamma \cdot I$ and $\bar{S}' = Q^{\prime T}\cdot L^{\prime -1}\cdot Q' + \gamma \cdot I$.
                ...
                \end{equation}
                This kernel restricted to a subgraph can also be used as a base kernel for one at a larger scale.
                ...
                More details are in the original paper~\parencite{kondor2016multiscale}.
            ...
            \paragraph{Propagation kernel}
                ...
                \end{equation}
                where $\kappa$ is a base kernel on node attributes (and/or labels).\\
                ...
                The latter, if not given by the user, is taken to be $P$.
                ...
                By binning values $h(u)$ in a graph $G$ and encoding the results in a vector $b_h(G)$, the simple graph kernel can be expressed as:
                \begin{equation}
                ...
                where:
                \begin{conditions}
                    \Sigma & is the $\mathbb{R}^{d_V \times d_V}$ covariance matrix computed from the distribution of attributes $\left(a(v)\right)_{v\in V}$.
                \end{conditions}
                To propagate these distributions at the next iteration, The mixture coefficients are propagated using $T$: $W_{t+1} = T\cdot W_t$.
                At initialization, $W_0 = I$ and hence $W_t= T^t$.

            \paragraph{Graph hopper kernel}
                ~\parencite{feragen2013scalable}.
                %https://ysig.github.io/GraKeL/dev/kernels/graph_hopper.html#feragen2013scalable
