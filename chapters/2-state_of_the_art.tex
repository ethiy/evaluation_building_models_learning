...
\section{\textsc{Supervised learning and pattern recognition}}
    ...
    \subsection{\textsc{Supervised classifiers}}
        ...
        \subsubsection{\textsc{\acrlong*{acr::svm}}}
            ...
            \paragraph{Hard margin}
                ...
                In order to compute the margin, we can interepret it as the length of the orthogonal projection of any vector $\bm{v}_{st}$\footnote{$\forall (s, t) \in \{1,2,\dots,n_{nsp}\} \times \{1,2,\dots,n_{psp}\}$} going from a negative support vector $\bm{x}^-_s$ to a positive one $\bm{x}^+_t$ on any line carried by $\bm{w}$.
            ...
            \paragraph{Properties}
                ...
                \begin{itemize}
                    \item No local optima and Can be efficiently solved.
                    \item Soft margin \gls{acr::svm} with a right tuning of the slackness parameter $C$ can achieve a good generalization;
                    \item It can handle large dimensional data
                    \item It can be adapted to an online setting using always the \gls{acr::smo} algorithm~\parencite{bordes2005fast}.
                    \item Can handle unbalanced datasets.
                \end{itemize}
                On the other hand it suffers also from some drawbacks, listed hereafter:
                \begin{itemize}
                    \item With a right engineered kernel one can introduce expert knowledge.
                    \item Parameter tunning is not straight-forward especially for kernels.
                    \item Natively designed for binary classification.
                    \item probability is not natively integrated and it is unreliable.
                \end{itemize}
        ...
        \subsubsection{\textsc{Random forest}}
            ...
            \paragraph{Decision tree classifier}
                ...
                The intuitive representation of decision tree classiers, shown in Figure~\ref{fig::decision_tree_graph}, is generaly prefered.
                ...
                The goals is to have leafs with no prediction errors.
            ...
            \paragraph{Bagging decision trees}
                ...
                \begin{equation}
                    ...
                    \end{aligned}.
                \end{equation}
                Multiple decision trees are trained where each one hopeful specializes in a certain pattern.
    ...
    \subsection{\textsc{\acrlong*{acr::scatnet} for image classification}}
    ...
    \subsection{\textsc{kernels for graph classification}}
