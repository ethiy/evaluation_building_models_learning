            ...
            \begin{center}
                \floatbox{figure}{
                    \begin{subfloatrow}[2]
                        ...
                        \quad
                        \ffigbox[\FBwidth]{
                                \caption{}\label{fig::seg_inter}
                            }
                            {\includestandalone[mode=buildnew, width=.45\textwidth]{figures/features/radiometric_features}
                        }
                    \end{subfloatrow}
                }{
                    \caption{Illustration of how features are derived from optical images. Model facets (each represented by a specific color) are projected onto  the aerial image (a). (b) local gradients (in purple), on intersecting pixels (in green), are compared to the edge (in red) normal (in black).}\label{fig::image_based}
                }
            \end{center}
        \end{figure}
