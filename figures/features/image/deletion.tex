...
\begin{document}
	\begin{tikzpicture}
        ...
        \path (model.north) node[anchor=south, text width=4cm] (model_legend) {(a) \acrshort{acr::3d} model.};
		\path (mask.north) node[anchor=south, text width=4cm] (mask_legend) {(b) Nadir Projection.};
        \path (image |- red_mask_channel.south) node[anchor=north, text width=4cm] (image_legend) {(c) Orthoimage.};
        \path (green_channel |- red_mask_channel.south) node[anchor=north, text width=4cm] (rgb_legend) {(d) RGB channels.};
        \path (green_mask_channel.south |- red_mask_channel.south) node[anchor=north, text width=4cm] (deletion_legend) {(e) Early fusion: \texttt{deletion}.};
	\end{tikzpicture}
\end{document}
