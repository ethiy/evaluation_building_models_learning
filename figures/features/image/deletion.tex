...
\begin{document}
	\begin{tikzpicture}
        ...
        \path (mask.north) node[anchor=south, text width=3cm] (mask_legend) {\footnotesize \subcaption{Nadir Projection. \label{subfig::nadir_d}}};
        \path (model |- mask_legend.north) node[anchor=north, text width=3cm] (model_legend) {\footnotesize \subcaption{\gls{acr::3d} model. \label{subfig::input_d}}};
        ...
        \path (green_mask_channel.south |-red_mask_channel.south) node[anchor=north, text width=3cm] (deletion_legend) {\footnotesize \subcaption{Early fusion: \texttt{deletion}. \label{subfig::early}}};

        \path (green_channel |- deletion_legend.south) node[anchor=south, text width=3cm] (rgb_legend) {\footnotesize \subcaption{RGB channels. \label{subfig::rgb_d}}};
        \path (image |- deletion_legend.south) node[anchor=south, text width=3cm] (image_legend) {\footnotesize \subcaption{Orthoimage. \label{subfig::orthoimage_d}}};
