...
\begin{document}
	\begin{tikzpicture}
        ...
        \path (model |- mask_legend) node[anchor=south, text width=3cm] (image_legend) {\footnotesize \subcaption{\gls{acr::3d} model. \label{subfig::input_d}}};

		\path (green_channel |- model) node (mask) {\includegraphics[width=3cm]{images/features/scatnet/elancourt-39322_mask}};
        \path (mask.north) node[anchor=south, text width=3cm] (image_legend) {\footnotesize \subcaption{Nadir Projection. \label{subfig::nadir_d}}};
        ...
        \path (green_channel.south |- deletion_legend) node[text width=3cm] (rgb_legend) {\footnotesize \subcaption{RGB channels. \label{subfig::rgb_d}}};
        \path (image.south |- deletion_legend) node[anchor=north, text width=3cm] (image_legend) {\footnotesize \subcaption{Orthoimage. \label{subfig::orthoimage_d}}};
