...
\begin{document}
    \begin{tikzpicture}
        ...
        \path (residual.west) + (-.4,0) node[anchor=east] (facet_graph) {\includestandalone[mode=buildnew, height=1cm]{figures/3d/building_graph}};
        \path (facet_graph |- residual_legend) node (facet_graph_legend) {\tiny (b) Geometric};

        \path (facet_graph.west) + (-1,0) node[anchor=east] (model) {\includestandalone[mode=buildnew, height=1cm]{figures/3d/building_model}};
		...
		\path (orthoprojection_legend.east |- ortho_projection) + (1,0) node[anchor=west] (errors) {\includestandalone[mode=buildnew, height=1cm]{figures/3d/building_errors}};
