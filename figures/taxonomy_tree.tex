...
\begin{document}
    ...
    \begin{tikzpicture}[grow=right]
        ...
        \path (b_under |- valid.north) + (-1,1) node (atomic_max) {$\texttt{finesse}=3$};
        \path (facet_errors |- input) + (-1,0) node (lod_max) {};
        \path (b_under |- input) + (7.4,0) node (lod_min) {};
        
        \draw[dashed, orange, ultra thick] (unqualifiable |- b_under.south) + (-1,-.5) -- (qualifiability_max);
        \draw[dashed, green, ultra thick] (valid |- b_under.south) + (-.75,-.5) -- (correctness_max);
        \draw[dashed, purple, ultra thick] (facet_errors |- b_under.south) + (-1,-.5) -- (families_max);
        \draw[dashed, blue, ultra thick] (b_under.south) + (-1,-.5) -- (atomic_max);
