...
\begin{document}
    ...
    \begin{tikzpicture}[grow=right]
        ...
        \path (valid.north) + (-.75,1) node[opacity=0] (correctness_max) {$\texttt{finesse}=1$};
        \path (facet_errors |- valid.north) + (-1,1) node[opacity=0] (families_max) {$\texttt{finesse}=2$};
        ...
        \draw[dashed, green, ultra thick, opacity=0] (valid |- b_under.south) + (-.75,-1) -- (correctness_max);
        \draw[dashed, purple, ultra thick, opacity=0] (facet_errors |- b_under.south) + (-1,-1) -- (families_max);
