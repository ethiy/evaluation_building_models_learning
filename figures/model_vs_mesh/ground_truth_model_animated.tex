...
\begin{document}
    \begin{tikzpicture}
        ...
        \path (3d) + (1, 1.875) node[darkgreen, visible on=<4->] (facet) {\scriptsize Facet $\equiv$ architectural feature};
        \path ($(facet.west) ! .8 ! (facet_point)$) node (halfway_facet_point) {};
        \path[draw, ->, darkgreen, visible on=<4->] (facet.west) -- (halfway_facet_point |- facet.west) -- (facet_point);
