...
\begin{document}
    ...
    \begin{tikzpicture}[grow=right]
        ...
        \path (families_max |- input) node[visible on=<5->] (lod_max) {};
        \path (b_under |- input) + (9.2,0) node[visible on=<5->] (lod_min) {};
        ...
        \draw[dash dot, ultra thick, visible on=<5->] (lod_max)  -- (lod_min) node[below, pos=.9] {\footnotesize \acrshort*{acr::lod}-0 $\cup$ \acrshort*{acr::lod}-1} node [above, pos=.9] {\footnotesize \acrshort*{acr::lod}-2 $\cup$ \acrshort*{acr::lod}-3};

        \path (families_max |- b_under.south) + (0,-1) node[visible on=<7->] (startbraces) {};
        \path (lod_min |- b_under.south) + (0,-1) node[visible on=<7->] (endbraces) {};
        \draw[decorate, decoration={brace, amplitude=5pt}, visible on=<7->] (b_under.south) + (-1,-1) -- (startbraces) node[midway, yshift=-2em, align=center]{Family errors:\\possible \textbf{exclusivity}};
        \draw[decorate, decoration={brace, amplitude=5pt,mirror}, visible on=<8->] (b_under.south) + (-1,-1) -- (endbraces) node[midway, yshift=-2em, align=center]{\texttt{Atomic} errors:\\no \textbf{exclusivity}};
    \end{tikzpicture}
\end{document}
