...
\begin{document}
    \begin{tikzpicture}
        ...
        \path (residual.west) + (-.4,0) node[anchor=east] (facet_graph) {\includestandalone[mode=tex, height=1cm]{figures/graphical_abstract/building_graph}};
        \path (facet_graph |- residual_legend) node (facet_graph_legend) {\tiny (b) Geometric};

        \path (facet_graph.west) + (-1,0) node[anchor=east] (model) {\includestandalone[mode=tex, height=1cm]{figures/graphical_abstract/building_model}};
		...
		\path (orthoprojection_legend.east |- ortho_projection) + (1,0) node[anchor=west] (errors) {\includestandalone[mode=tex, height=1cm]{figures/graphical_abstract/building_errors}};
